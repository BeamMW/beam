...
namespace beam::wallet
{
    ...
    BaseTransaction::Ptr AssetIssueTransaction::Create(INegotiatorGateway& gateway
        , IWalletDB::Ptr walletDB
        , IPrivateKeyKeeper::Ptr keyKeeper
        , const TxID& txID)
    {
        return BaseTransaction::Ptr(new AssetIssueTransaction(gateway, walletDB, keyKeeper, txID));
    }

    AssetIssueTransaction::AssetIssueTransaction(INegotiatorGateway& gateway
                                        , IWalletDB::Ptr walletDB
                                        , IPrivateKeyKeeper::Ptr keyKeeper
                                        , const TxID& txID)
    ...
    void AssetIssueTransaction::UpdateImpl()
    {
        ...
        if (!m_TxBuilder)
        {
            m_TxBuilder = make_shared<AssetIssueTxBuilder>(*this, kDefaultSubTxID, amoutList, GetMandatoryParameter<Amount>(TxParameterID::Fee), _assetId, _skAsset);
        }
         ...
         || (!isSender && !builder.HasKernelID()))
        {
            ...
            if (!builder.GetInitialTxParams() && txState == State::Initial)
            {
                ...
                if (isSelfTx || !isSender)
                {
                    ...
                    {
                        builder.GenerateAssetCoin(amount, _assetId);
                    }
                }
    ...
    void AssetIssueTransaction::SendInvitation(const AssetIssueTxBuilder& builder, bool isSender)
    {
        SetTxParameter msg;
    ...
    void AssetIssueTransaction::ConfirmInvitation(const AssetIssueTxBuilder& builder, bool sendUtxos)
    {
    ...
    void AssetIssueTransaction::ConfirmTransaction(const AssetIssueTxBuilder& builder, bool sendUtxos)
    {
        uint32_t nVer = 0;
        if (GetParameter(TxParameterID::PeerProtoVersion, nVer))
        {
    ...
    void AssetIssueTransaction::NotifyTransactionRegistered()
    {
        SetTxParameter msg;
		uint8_t nCode = proto::TxStatus::Ok; // compiler workaround (ref to static const)
    ...
    bool AssetIssueTransaction::IsSelfTx() const
    {
