...
void NodeConnection::Accept(io::TcpStream::Ptr&& newStream)
{
    ...
    newStream->enable_keepalive(Rules::get().DA.get_Target_s() + 1); // it should be comparable to the block rate
...
void NodeConnection::ProvePKdfObscured(Key::IPKdf& kdf, uint8_t nIDType)
{
        ...
        :public Key::IKdf
    {
        ...
        void DerivePKey(ECC::Scalar::Native&, const ECC::Hash::Value&) override
        {
            assert(false);
        }

        void DeriveKey(ECC::Scalar::Native& out, const ECC::Hash::Value& hv) override
        {
...
bool NodeConnection::IsPKdfObscured(Key::IPKdf& kdf, const PeerID& id)
{
        ...
        :public Key::IPKdf
    {
        ...
        void DerivePKeyG(ECC::Point::Native& out, const ECC::Hash::Value& hv) override
        {
            ECC::Scalar::Native s;
        ...
        void DerivePKeyJ(ECC::Point::Native& out, const ECC::Hash::Value& hv) override
        {
            assert(false);
        }
