...
{
    // current version
    static const ShaderID s_SID = { 0xab,0xdf,0xbd,0x7e,0x4a,0x5e,0x25,0x68,0x77,0xe0,0x4a,0xb1,0xfd,0xe2,0xa7,0xbd,0xdb,0xd2,0x7b,0xb3,0x6f,0x6c,0xdc,0xf3,0xbe,0xe4,0x80,0x3e,0x07,0x28,0x29,0x25 };
...
#define DemoXdao_manager_my_admin_key(macro)

#define DemoXdao_manager_deploy_contract(macro) Upgradable2_deploy(macro)

#define DemoXdao_multisig_args(macro) \
...
{
    Amount valReceived = DemoXdao::Preallocated::s_Emission;
    ...
    Amount valAvail = CalculatePreallocAvail(cid, DemoXdao::Preallocated::s_Emission);

    Env::DocAddNum("total", DemoXdao::Preallocated::s_Emission);
