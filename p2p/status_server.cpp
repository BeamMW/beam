...
StatusServer::StatusServer(io::Reactor& reactor, io::Address bindAddress, BlockCallback blockCallback) :
    _msgCreator(2000),
    _reactor(reactor),
    _timers(reactor, 100),
    _bindAddress(bindAddress),
    _blockCallback(std::move(blockCallback))
{
...
void StatusServer::update_status(const DummyStatus& ds) {
    _statusBody = dump_to_json(_msgCreator, ds);
}

void StatusServer::start_server() {
    try {
...
void StatusServer::on_stream_accepted(io::TcpStream::Ptr&& newStream, io::ErrorCode errorCode) {
    if (errorCode == 0) {
...
bool StatusServer::on_request(uint64_t id, const HttpMsgReader::Message& msg) {
...
bool StatusServer::send_status(const HttpConnection::Ptr& conn) {
    return send(conn, 200, "OK", &_statusBody);
}

bool StatusServer::send_blocks(const HttpConnection::Ptr& conn) {
...
bool StatusServer::send_404(const HttpConnection::Ptr& conn) {
    return send(conn, 404, "Not Found", 0);
}

bool StatusServer::send(const HttpConnection::Ptr& conn, int code, const char* message, const io::SharedBuffer* body) {
    assert(conn);
