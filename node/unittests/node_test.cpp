		...
		void Test()
		{
...

			{
				...
				for (size_t i = 0; i < _countof(pLC); i++)
				{
					...
					pE->m_pResponse.reset(new Treasury::Response);
					uint64_t nIndex = 1;
					verify_test(pE->m_pResponse->Create(pE->m_Request, *pLC[i].m_pKdf, nIndex));
			...
			Rules::get().DA.Difficulty0 = Difficulty(3 << Difficulty::s_MantissaBits); // 2^3 = 8;
			...
			for (size_t i = 0; i < _countof(pLC); i++)
			{
				pLC[i].m_Conn.m_Cfg.m_vNodes.push_back(io::Address(INADDR_LOOPBACK, g_Port));
				pLC[i].Initialize();
			}
