...
bool CQR_Encode::EncodeData(int nLevel, int nVersion, bool bAutoExtent, int nMaskingNo, char* lpsSource, int ncSource)
{
	...
	int ncLength = ncSource > 0 ? ncSource : static_cast<int>(strlen(lpsSource));
	...
	for (i = 0; i < m_ncDataBlock && m_ncDataCodeWordBit != -1; ++i)
	{
		if (m_byBlockMode[i] == QR_MODE_NUMERAL)
		{
			...
			for (j = 0; j < m_nBlockLength[i]; j += 3)
			{
				...
				{
                    unsigned short wBinCode = (unsigned short)(((lpsSource[ncComplete + j]	  - '0') * 100) +
				...
				{
                    unsigned short wBinCode = (unsigned short)(((lpsSource[ncComplete + j] - '0') * 10) +
				...
				{
                    unsigned short wBinCode = (unsigned short)(lpsSource[ncComplete + j] - '0');
		...
		else if (m_byBlockMode[i] == QR_MODE_ALPHABET)
		{
			...
			for (j = 0; j < m_nBlockLength[i]; j += 2)
			{
				...
				{
                    unsigned short wBinCode = (unsigned short)((AlphabetToBinaly(lpsSource[ncComplete + j]) * 45) +
				...
				{
                    unsigned short wBinCode = (unsigned short)AlphabetToBinaly(lpsSource[ncComplete + j]);
