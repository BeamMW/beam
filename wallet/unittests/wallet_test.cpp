...
namespace
{
    ...
    void TestP2PWalletNegotiationST()
    {
        ...
        // rollback test
        {
            ...
            receiver.m_WalletDB->get_History().DeleteFrom(sTip.get_Height()); // delete latest block
    ...
    void TestTxRollback()
    {
        ...
        TestNode node;
        {
            ...
            sender.m_WalletDB->get_History().DeleteFrom(tip.get_Height());
    ...
    void TestAppShader2(Rules& r)
    {
        ...
        for (uint32_t i = 0; i < nPeers; i++)
        {
            ...
            pMan[i]->m_Args["hTarget"] = std::to_string(node.get_Processor().m_Cursor.m_hh.m_Height + 50);
        ...
        Height hTarget = node.get_Processor().m_Cursor.m_hh.m_Height + 20;
        ...
        if (node.get_Processor().m_Cursor.m_hh.m_Height < hTarget)
        {
            struct MyObserver :public Node::IObserver
            {
                ...
                void OnStateChanged() override {
                    if (m_Node.get_Processor().m_Cursor.m_hh.m_Height >= m_hTarget)
                        io::Reactor::get_Current().stop();
                }
    ...
    void TestAppShader3(Rules& r)
    {
        ...
        Height hTarget = node.get_Processor().m_Cursor.m_hh.m_Height + 15;
...
struct TreasuryUser
{
    ...
    {
        ECC::HKdf m_kdf;
        uint32_t m_MinerSubkey;
        Key::IKdf::Ptr m_pMiner;
        PeerID m_ValidatorAddr;
    };

    void InitMaster(Keys& keys) const
    {
        WordList wl;

        for (const char* szPhrase = m_Mnemonic; ; )
        {
        ...
        keys.m_kdf.Generate(seed.hash().V);

        keys.m_MinerSubkey = 1;
        keys.m_pMiner = MasterKey::get_Child(keys.m_kdf, keys.m_MinerSubkey);

        ECC::Scalar::Native sk;
        Block::Pbft::DeriveValidatorAddress(*keys.m_pMiner, keys.m_ValidatorAddr, sk);

    }
};

const TreasuryUser g_pTreasuryUser[] = {
    { "Anatol",              "drum;number;north;fly;silk;recall;execute;season;december;foot;spirit;tennis;",         },
    { "Sergei",              "drill;that;swamp;garbage;forward;cargo;punch;dinner;crumble;credit;sample;hungry;",     },
    { "bigromanov",          "drama;toddler;end;wealth;foot;unique;shed;lunar;enter;nuclear;choose;music;",           },
    { "vain onnellinen",     "spare;cousin;soap;used;claw;pepper;silver;neutral;sword;open;print;picnic;",            },
    { "Vika",                "alien;sand;rocket;together;pyramid;rib;spice;spray;diesel;opinion;idea;snap;",          },
    { "Artem",               "happy;rely;throw;liar;robust;assume;industry;earth;fringe;ensure;possible;run;",        },
    { "Vadim",               "essay;want;water;firm;affair;garment;rally;leaf;body;luggage;wheat;promote;",           },
    { "Roman",               "early;arena;door;admit;route;snow;forward;census;ramp;update;supreme;rough;",           },
    { "Sasha",               "around;slender;post;cactus;finish;hard;drum;symptom;future;custom;pen;dune;",           },
    { "Alexander",           "slogan;leg;sudden;gloom;excuse;evidence;surround;dismiss;onion;naive;gap;quality;",     },
    { "Vladik",              "rebuild;stem;outside;chuckle;execute;vendor;member;video;cloth;deposit;mask;pumpkin;",  },
    { "Denis",                "tonight;ship;rule;brown;member;nose;desk;describe;island;cement;rubber;slab;"           },
};
...
void MakeTreasury_dappnet2()
{
	...
	for (uint32_t i = 0; i < _countof(g_pTreasuryUser); i++)
	{
        const auto& u = g_pTreasuryUser[i];

        TreasuryUser::Keys keys;
        u.InitMaster(keys);

        PeerID pid;
        ECC::Scalar::Native sk;
        Treasury::get_ID(keys.m_kdf, pid, sk);

        std::cout << u.m_Name << std::endl;
        std::cout << "\t" << u.m_Mnemonic << std::endl;

        std::cout << "\tTr ID: " << pid.full() << std::endl;

        ks.m_sMeta = "0";
        ks.ExportP(keys.m_kdf);
        std::cout << "\tOwner key: " << ks.m_sRes << std::endl;

        ks.m_sMeta = std::to_string(keys.m_MinerSubkey);
        ks.ExportS(*keys.m_pMiner);
        std::cout << "\tMiner key: " << ks.m_sRes << std::endl;

        std::cout << "\tValidator: " << keys.m_ValidatorAddr.full() << std::endl;
        ...
        uint64_t nIndex = 1;
        WALLET_CHECK(pE->m_pResponse->Create(pE->m_Request, keys.m_kdf, nIndex));

        if (i + 1 == _countof(g_pTreasuryUser))
            tb.AddValidator(keys.m_ValidatorAddr, Rules::Coin * 5000);
	}
...
void MakeTreasury_warp()
{
    ...
    std::cout << "warp PBFT_DPOS cid " << tb.m_Cid.full() << std::endl;

    struct MyMan
        :public bvm2::ProcessorManager
    {
        static void get_DelegatorPreimage(ECC::Hash::Value& hv, const bvm2::ContractID& cid)
        {
            DeriveKeyPreimage(hv, cid);
        }
    };

    ECC::Hash::Value hvDelegator;
    MyMan::get_DelegatorPreimage(hvDelegator, tb.m_Cid);
    ...
    for (uint32_t i = 0; i < _countof(g_pTreasuryUser); i++)
    {
        ...
        ECC::Point::Native ptN;
        keys.m_kdf.DerivePKeyG(ptN, hvDelegator);
        ECC::Point ptD = ptN;

        std::cout << "\tDelegator: " << Cast::Reinterpret<uintBig_t<ECC::uintBig::nBytes + 1>>(ptD).str() << std::endl;

        if (i < 4)
            tb.AddValidator(keys.m_ValidatorAddr, ptD, Rules::Coin * 5000);
    }
    ...
    FStream fs;
    fs.Open("warp_treasury.bin", false, true);
    fs.write(&buf.front(), buf.size());

    std::cout << "warp treasury checksum: " << hv.full() << std::endl;
}
