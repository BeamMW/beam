...
namespace beam::wallet
{
    ...
    AssetIssueTxBuilder::AssetIssueTxBuilder(BaseTransaction& tx, SubTxID subTxID, const AmountList& amountList, Amount fee, beam::AssetID assetId, ECC::Scalar::Native skAsset)
        : m_Tx{ tx }
        , m_SubTxID(subTxID)
    ...
    void AssetIssueTxBuilder::GenerateChangeCoin(beam::Amount amount)
    {
        Coin newUtxo{ amount };
    ...
    void AssetIssueTxBuilder::GenerateAssetCoin(Amount amount, beam::AssetID assetId)
    {
        Coin newUtxo{ amount };
        newUtxo.m_createTxId = m_Tx.GetTxID();
        m_Tx.GetWalletDB()->storeCoin(newUtxo);
