...
#define Test_manager_create(macro)
#define Test_manager_view(macro)
#define Test_manager_destroy(macro) macro(ContractID, cid)
#define Test_manager_view_accounts(macro) macro(ContractID, cid)
#define Test_manager_view_logs(macro) macro(ContractID, cid)

#define Test_manager_view_account(macro) \
    macro(ContractID, cid) \
    macro(PubKey, pubKey)

#define Test_Role_manager(macro) \
...
//#define Test_my_account_view(macro) macro(ContractID, cid)
//#define Test_my_account_get_proof(macro) \
//    macro(ContractID, cid) \
//    macro(AssetID, aid)
//
//#define Test_my_account_deposit(macro) \
//    macro(ContractID, cid) \
//    macro(Amount, amount) \
//    macro(AssetID, aid)

//#define Test_my_account_withdraw(macro) Test_my_account_deposit(macro)

//#define Test_my_account_move(macro) \
//    macro(uint8_t, isDeposit) \
//    Test_my_account_deposit(macro)
//
//#define Test_Role_my_account(macro) \
//    macro(my_account, view) \
//    macro(my_account, get_proof) \
//    macro(my_account, deposit) \
//    macro(my_account, withdraw)

#define Test_Roles_All(macro) \
    macro(manager) //\
    //macro(my_account)
...
#define THE_METHOD(role, name) { Env::DocGroup grMethod(#name);  Test_##role##_##name(THE_FIELD) }
#define THE_ROLE(name) { Env::DocGroup grRole(#name); Test_Role_##name(THE_METHOD) }
        
        Test_Roles_All(THE_ROLE)
#undef THE_ROLE
#undef THE_METHOD
#undef THE_FIELD
    }
...
#define ON_METHOD(role, name) void On_##role##_##name(Test_##role##_##name(THE_FIELD) int unused = 0)
...
{
    Env::GenerateKernel(&cid, 1, nullptr, 0, nullptr, 0, nullptr, 0, "destroy Test contract", 0);
}
...
#define THE_METHOD(role, name) \
        if (!Env::Strcmp(szAction, #name)) { \
            Test_##role##_##name(PAR_READ) \
            On_##role##_##name(Test_##role##_##name(PAR_PASS) 0); \
            return; \
        }

#define THE_ROLE(name) \
    if (!Env::Strcmp(szRole, #name)) { \
        Test_Role_##name(THE_METHOD) \
