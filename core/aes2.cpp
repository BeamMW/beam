...
// Based on this implementation: https://github.com/SergeyBel/AES
// Modified a little to make it more suitable for the case (removed dyn allocs mostly)
...
namespace AES2 {
	...
	void Coder::Init256(const uint8_t* pKey)
	{
		m_Nk = 8;
		m_Nr = 14;
		InitInternal(pKey);
	...
	void Coder::Init192(const uint8_t* pKey)
	{
		m_Nk = 6;
		m_Nr = 12;
		InitInternal(pKey);
	...
	void Coder::Init128(const uint8_t* pKey)
	{
		m_Nk = 4;
		m_Nr = 10;
		InitInternal(pKey);
	...
	void Coder::InitInternal(const uint8_t* pKey)
	{
		assert(m_Nr <= Nr_Max);
		...
		while (i < 4 * Nb * (m_Nr + 1)) {
			...
			m_pK[i + 0] = m_pK[i - 4 * m_Nk] ^ temp[0];
			m_pK[i + 1] = m_pK[i + 1 - 4 * m_Nk] ^ temp[1];
			m_pK[i + 2] = m_pK[i + 2 - 4 * m_Nk] ^ temp[2];
			m_pK[i + 3] = m_pK[i + 3 - 4 * m_Nk] ^ temp[3];
			i += 4;
		}
	}

	static const uint32_t Nb = Coder::Nb;
	...
	void Coder::EncodeBlock(uint8_t* pDst, const uint8_t* pSrc) const
	{
		...
		for (i = 0; i < 4; i++) {
			for (j = 0; j < Nb; j++) {
				state[i][j] = pSrc[i + 4 * j];
			}
		}

		AddRoundKey(state, m_pK);

		for (round = 1; round <= m_Nr - 1; round++) {
			SubBytes(state);
			ShiftRows(state);
			MixColumns(state);
			AddRoundKey(state, m_pK + round * 4 * Nb);
		}

		SubBytes(state);
		ShiftRows(state);
		AddRoundKey(state, m_pK + m_Nr * 4 * Nb);

		for (i = 0; i < 4; i++) {
			for (j = 0; j < Nb; j++) {
				pDst[i + 4 * j] = state[i][j];
			}
		}
	}
	...
	void Coder::DecodeBlock(uint8_t* pDst, const uint8_t* pSrc) const
	{
		unsigned char state[4][Nb];
		unsigned int i, j, round;
		...
		for (round = m_Nr - 1; round >= 1; round--) {
			InvSubBytes(state);
			InvShiftRows(state);
			AddRoundKey(state, m_pK + round * 4 * Nb);
			InvMixColumns(state);
