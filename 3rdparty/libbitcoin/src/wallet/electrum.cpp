...
static size_t special_modulo(int32_t index_distance, size_t size)
{
...
{
    const auto normal = to_chunk(join(mnemonic));
...
word_list create_mnemonic(const data_chunk& entropy, seed prefix)
{
...
bool validate_mnemonic(const word_list& mnemonic, seed prefix)
{
    const dictionary lexicon = libbitcoin::wallet::language::electrum::en;
